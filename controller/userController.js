...
const storage = new Storage({
  projectId: "test-b0acd",
  keyFilename: path.resolve(
    "./test-b0acd-firebase-adminsdk-47ci8-1d00646397.json"
  ),
});
const bucket = storage.bucket("test-b0acd.appspot.com");
...
exports.deleteUser = async (req, res, next) => {
  try {
    ...
    if (!user) {
      const err = new Error("ไม่พบผู้ใช้ที่ต้องการลบ");
      err.statusCode = 404;
      throw err;
    }
    res.status(200).json({
...
exports.getPagination = async (req, res, next) => {
  try {
    const page = parseInt(req.query.page) || 1; //หน้าที่ต้องการ เช่น 1 2 3
